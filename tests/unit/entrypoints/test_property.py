from datetime import UTC, datetime
from uuid import UUID

from fastapi import FastAPI
from fastapi.testclient import TestClient

from woonlens.application.services.addresses import AddressService
from woonlens.application.services.property import PropertyDetailsService
from woonlens.bootstrap.settings import Settings
from woonlens.domain.addresses import (
    AddressSuggestion,
    Coordinates,
    ResolvedAddress,
    SourceMetadata,
)
from woonlens.domain.property import Building, PropertyDetails, ResidentialUnit
from woonlens.entrypoints.api import create_app

ADDRESS_ID = UUID("11111111-1111-4111-8111-111111111111")
SOURCE = SourceMetadata(
    "PDOK / Kadaster",
    "BAG",
    datetime(2026, 8, 31, tzinfo=UTC),
    "Public Domain Mark 1.0",
)


class SearchFake:
    async def suggest(self, query: str, *, limit: int) -> tuple[AddressSuggestion, ...]:
        del query, limit
        return ()


class DetailsFake:
    async def resolve(self, address_id: UUID) -> ResolvedAddress:
        return ResolvedAddress(
            address_id,
            "number",
            "0599010000295420",
            "Verblijfsobject",
            "Street",
            "1",
            None,
            None,
            "1234AB",
            "City",
            Coordinates(4.9, 52.37),
            SOURCE,
        )


class PropertyFake:
    async def fetch(self, addressable_object_id: str) -> PropertyDetails:
        assert addressable_object_id == "0599010000295420"
        return PropertyDetails(
            ResidentialUnit(
                addressable_object_id,
                "Verblijfsobject in gebruik",
                ("woonfunctie",),
                62,
            ),
            (
                Building(
                    "0599100000691863", "Pand in gebruik", 1873, ("woonfunctie",), 4
                ),
            ),
            SOURCE,
        )


def create_test_app() -> FastAPI:
    details = DetailsFake()
    return create_app(
        Settings(environment="test"),
        AddressService(SearchFake(), details, suggestion_limit=8),
        property_details_service=PropertyDetailsService(details, PropertyFake()),
    )


def test_endpoint_returns_labeled_property_details() -> None:
    with TestClient(create_test_app()) as client:
        response = client.get(f"/api/v1/addresses/{ADDRESS_ID}/property")

    assert response.status_code == 200
    body = response.json()
    assert body["residential_unit"]["registered_area_m2"] == 62
    assert body["residential_unit"]["area_definition"] == "BAG registered area"
    assert body["buildings"][0]["construction_year"] == 1873
    assert body["source"]["license"] == "Public Domain Mark 1.0"


def test_endpoint_rejects_non_uuid_address() -> None:
    with TestClient(create_test_app()) as client:
        response = client.get("/api/v1/addresses/not-a-uuid/property")
    assert response.status_code == 422
